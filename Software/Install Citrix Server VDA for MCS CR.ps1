...
<#
.SYNOPSIS
This script installs Citrix Multisession VDA CR on a MCS/PVS master server/client or wherever you want.
	...
	IF (!(Test-Path "$InstDir\Software\Citrix\Current\CVAD")) {
		...
		IF ($VDA -lt $VersionVDA) {
			try	{
				...
				Start-Process "$PSScriptRoot\Citrix\Current\CVAD\x64\XenDesktop Setup\XenDesktopVdaSetup.exe" –ArgumentList "/NOREBOOT /exclude ""Citrix Personalization for App-V - VDA"",""Citrix Telemetry Service"",""Citrix VDA Upgrade Agent"" /includeadditional ""Citrix MCS IODriver"" /COMPONENTS UBERAGENT,VDA,DT /disableexperiencemetrics /ENABLE_REMOTE_ASSISTANCE /ENABLE_HDX_PORTS /ENABLE_HDX_UDP_PORTS /ENABLE_REAL_TIME_TRANSPORT /enable_ss_ports /mastermcsimage" –NoNewWindow -Wait
				DS_WriteLog "-" "" $LogFile
