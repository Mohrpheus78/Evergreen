...
IF (Test-Path -Path "$PSScriptRoot\$Product\Version.txt") {
	...
	$SQLMGMT = (Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "**Microsoft SQL Server Management**"}).DisplayVersion | Select-Object -Last 1
	...
	try	{
		Start-Process "$PSScriptRoot\$Product\SSMS-Setup-DEU.exe" –ArgumentList '/install /passive /quiet /norestart' –NoNewWindow -Wait
		DS_WriteLog "-" "" $LogFile
