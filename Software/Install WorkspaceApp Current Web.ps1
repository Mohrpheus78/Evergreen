...
IF (Test-Path -Path "$PSScriptRoot\MS Edge WebView2 Runtime\Version.txt") {
	...
	[version]$WSA = [version]$WSA = (Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "*Citrix Workspace*" -and $_.DisplayName -notlike "*Citrix Workspace Environment*"}).DisplayVersion | Select-Object -First 1
	...
	IF (!(Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -eq "Microsoft Windows Desktop Runtime - 8.0.11 (x86)"})) {
		...
		try {
			Start-Process -FilePath "$PSScriptRoot\Citrix\WorkspaceApp\Windows\LTSR\windowsdesktop-runtime-8.0.11-win-x86.exe" -ArgumentList "/quiet /noreboot" –NoNewWindow -wait
			DS_WriteLog "-" "" $LogFile
			Write-Host -ForegroundColor Green " ... ready!"
			Write-Output "" 
		} catch {
