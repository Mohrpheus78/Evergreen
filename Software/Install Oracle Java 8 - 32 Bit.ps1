...
IF (Test-Path -Path "$PSScriptRoot\$Product\Version.txt") {
	...
	$JavaVersion = (Get-ItemProperty HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "*Java 8*Update*"}).DisplayName
	IF ([string]::ISNullOrEmpty( $JavaVersion) -eq $False) {
	# $Subfolder = (Get-ChildItem -Path "C:\Program Files (x86)\Java").Name | Select-Object -First 1
	$Java32 = Get-Content -Path "C:\Program Files (x86)\Java\latest\jre-1.8\release" -TotalCount 1 -EA SilentlyContinue
	IF (!($Java32)) {
		$Java32 = Get-Content -Path "C:\Program Files (x86)\Java\jre-1.8\release" -TotalCount 1 -EA SilentlyContinue
		}
