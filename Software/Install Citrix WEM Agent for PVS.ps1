	...
	IF ($WEMAgent -lt $VersionWEM) {
		...
		try	{
			IF ($CC) {
				...
				Start-Process "$InstDir\Software\Citrix\Cloud\Citrix Workspace Environment Management Agent.exe" -ArgumentList '/quiet Cloud=1 AgentCacheLocation=D:\WEMCache CloudConnectorList=$CCList' –NoNewWindow -Wait
			}
			IF ($CVADConnectors) {
				Start-Process "$InstDir\Software\Citrix\Cloud\Citrix Workspace Environment Management Agent.exe" -ArgumentList '/quiet Cloud=1 AgentCacheLocation=D:\WEMCache' –NoNewWindow -Wait
			}
			DS_WriteLog "-" "" $LogFile
			write-Host -ForegroundColor Green "...ready"
			Write-Output ""
