...
Write-Host ""
	...
	try	{
		...
		Start-Process "$InstDir\Citrix\WEM\Citrix Workspace Environment Management Agent.exe" -ArgumentList '/quiet Cloud=0 AgentCacheLocation=D:\WEMCache InfrastructureServer=$WEMServer' –NoNewWindow -Wait
		} catch {
	...
	try	{
		...
		Start-Process "$InstDir\Citrix\Cloud\Citrix Workspace Environment Management Agent.exe" -ArgumentList '/quiet Cloud=1 AgentCacheLocation=D:\WEMCache CloudConnectorList=$CC' –NoNewWindow -Wait
		} catch {
		DS_WriteLog "E" "Error installing $Product (error: $($Error[0]))" $LogFile       
		}
	DS_WriteLog "-" "" $LogFile
