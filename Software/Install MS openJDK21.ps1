...
<#
.SYNOPSIS
This script installs MS openJDK21 on a MCS/PVS master server/client or wherever you want.
...
$arguments = @(
	...
	"INSTALLDIR=`"C:\Program Files\Microsoft\`""
)
if ($targetDir){
...
IF (Test-Path -Path "$PSScriptRoot\$Product\Version.txt") {
	...
	[version]$openJDK = (Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "Microsoft JDK*"}).DisplayVersion
	IF ($openJDK -lt $Version) {

	# MS openJDK
	Write-Host -ForegroundColor Yellow "Installing $Product"
	DS_WriteLog "I" "Installing $Product" $LogFile
	try {
		"$PSScriptRoot\$Product\MicrosoftOpenJDK.msi" | Install-MSIFile
			DS_WriteLog "-" "" $LogFile
