...
IF (Test-Path -Path "$PSScriptRoot\$Product\Version.txt") {
	...
	$VcRedist = (Get-ItemProperty HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "Microsoft Visual C++ 2022 x86*"}).DisplayVersion | Select-Object -First 1
	IF ([string]::ISNullOrEmpty( $VcRedist) -eq $True) {
		$VcRedist = (Get-ItemProperty HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "Microsoft Visual C++ 2019 x86*"}).DisplayVersion | Select-Object -First 1
		}
	$VcRedist = $VcRedist + ".0"
