...
IF (Test-Path -Path "$PSScriptRoot\$Product\Version.txt") {
	...
	Write-Host -ForegroundColor Yellow "Installing MS DotNet Desktop Runtime 10.0.8 (Prerequisite for Remotedesktopmanager)"
	DS_WriteLog "I" "Installing MS DotNet Desktop Runtime 10.0.8 (Prerequisite for Remotedesktopmanager)" $LogFile
	IF (!(Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -eq "Microsoft Windows Desktop Runtime - 9.0.4 (x64)"})) {
		try {
			Start-Process -FilePath "$PSScriptRoot\$Product\windowsdesktop-runtime-10.0.8-win-x64.exe" -ArgumentList "/quiet /noreboot" –NoNewWindow -wait
			DS_WriteLog "-" "" $LogFile
