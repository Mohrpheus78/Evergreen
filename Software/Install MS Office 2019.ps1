...
<#
.SYNOPSIS
This script installs MS Office 2019 on a MCS/PVS master server/client or wherever you want.
...
IF (Test-Path -Path "$PSScriptRoot\$Product\Version.txt") {
	...
	[version]$MS365Apps = (Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "Microsoft Office * 2019*"}).DisplayVersion | Select-Object -First 1
