	...
	IF (!(Test-Path "$InstDir\Software\Citrix\Current\CVAD")) {
		...
		IF ($VDA -lt $VersionVDA) {
			try	{
				...
				Start-Process "$InstDir\Software\Citrix\Current\CVAD\x64\XenDesktop Setup\XenDesktopVdaSetup.exe" –ArgumentList "/NOREBOOT /exclude ""Citrix Personalization for App-V - VDA"",""Machine Identity Service"",""Citrix Telemetry Service"",""Citrix MCS IODriver"",""Citrix Workspace Environment Management Agent"",""Citrix Backup and Restore"" /disableexperiencemetrics /ENABLE_REMOTE_ASSISTANCE /ENABLE_HDX_PORTS /ENABLE_HDX_UDP_PORTS /ENABLE_REAL_TIME_TRANSPORT /enable_hdx_tls_dtls /enable_ss_ports" –NoNewWindow -Wait
				DS_WriteLog "-" "" $LogFile
