...
<#
.SYNOPSIS
This script installs KeePassXC on a MCS/PVS master server/client or wherever you want.
...
IF (Test-Path -Path "$PSScriptRoot\$Product\Version.txt") {
	...
	[version]$KeePassXC = (Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\* | Where-Object {$_.DisplayName -like "KeePassXC*"}).DisplayVersion
	IF ($KeePassXC -lt $Version) {

	# KeePassXC
	Write-Host -ForegroundColor Yellow "Installing $Product"
	DS_WriteLog "I" "Installing $Product" $LogFile
	try {
		"$PSScriptRoot\$Product\KeePassXC.msi" | Install-MSIFile
		DS_WriteLog "-" "" $LogFile
